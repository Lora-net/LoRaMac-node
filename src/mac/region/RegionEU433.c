...
void RegionEU433ComputeRxWindowParameters( int8_t datarate, uint32_t rxError, RxConfigParams_t *rxConfigParams )
{
    ...
    if( datarate == DR_7 )
    { // FSK
        tSymbol = ( 1.0 / ( double )DataratesEU433[datarate] ) * 8.0; // 1 symbol equals 1 byte
    }
    else
    { // LoRa
        tSymbol = ( ( double )( 1 << DataratesEU433[datarate] ) / ( double )BandwidthsEU433[datarate] ) * 1e3;
    }
...
bool RegionEU433TxConfig( TxConfigParams_t* txConfig, int8_t* txPower, TimerTime_t* txTimeOnAir )
{
    ...
    phyTxPower = TxPowersEU433[txPowerLimited];
