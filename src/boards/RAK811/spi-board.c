...
uint8_t SpiTransfer(  Spi_t *obj, uint8_t data )
{
	while( __HAL_SPI_GET_FLAG( &obj->Spi, SPI_FLAG_TXE ) == RESET )
			;
	obj->Spi.Instance->DR = ( uint16_t ) ( data );
	while( __HAL_SPI_GET_FLAG( &obj->Spi, SPI_FLAG_RXNE ) == RESET )
		;
	data = ( uint16_t ) obj->Spi.Instance->DR;
...
uint16_t SpiInOut( Spi_t *obj, uint16_t outData )
{
	...
	else
	{
		...
		rxData = SpiTransfer( obj, (outData & 0xFF) );
...
void SpiOut( Spi_t *obj, uint8_t addr, uint8_t *buffer, uint8_t size )
{
    uint8_t i, x;
	((void)(x));
	...
	SpiTransfer( obj, (addr | 0x80) );

    for ( i = 0; i < size; i++ )
    {
		SpiTransfer( obj, *buffer++ );
    }
...
void SpiIn( Spi_t *obj, uint8_t addr, uint8_t *buffer, uint8_t size )
{
    uint8_t i;
	...
	SpiTransfer( obj, (addr & 0x7F) );

    for ( i = 0; i < size; i++ )
    {
		*buffer++ = SpiTransfer( obj, 0 );
    }
