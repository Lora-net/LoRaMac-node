...
{
    GpioInit( &RfSwitch, RF_RXTX, PIN_OUTPUT, PIN_PUSH_PULL, PIN_NO_PULL, 1 );
}

void SX1276AntSwDeInit( void )
{
    GpioInit( &RfSwitch, RF_RXTX, PIN_ANALOGIC, PIN_PUSH_PULL, PIN_NO_PULL, 0 );
}
...
void SX1276SetAntSw( uint8_t opMode )
{
    ...
    case RFLR_OPMODE_TRANSMITTER:
        GpioWrite( &RfSwitch, 0 );
        break;
    case RFLR_OPMODE_RECEIVER:
    case RFLR_OPMODE_RECEIVER_SINGLE:
    case RFLR_OPMODE_CAD:
    default:
        GpioWrite( &RfSwitch, 1 );
        break;
    }
}
