    ...
    case MODEM_FSK:
        {
            ...
            SX1272.Settings.Fsk.RxSingleTimeout = ( uint32_t )
    ...
    case MODEM_FSK:
        {
            uint64_t Num =
              ( uint64_t )8 *
                (
                  ( uint32_t )SX1272.Settings.Fsk.PreambleLen +
                  ( ( SX1272Read( REG_SYNCCONFIG ) & ~RF_SYNCCONFIG_SYNCSIZE_MASK ) + 1 ) +
                  ( ( SX1272.Settings.Fsk.FixLen == 0x01 ) ? 0 : 1 ) +
                  ( ( ( SX1272Read( REG_PACKETCONFIG1 ) & ~RF_PACKETCONFIG1_ADDRSFILTERING_MASK ) != 0x00 ) ? 1 : 0 ) +
                  pktLen +
                  ( ( SX1272.Settings.Fsk.CrcOn == 0x01 ) ? 2 : 0 )
                ) * 1000;
            airTime = INT_CLOSEST_TO_INFINITIES_DIV_BOTH_POSTITIVE_SIGNS(Num, SX1272.Settings.Fsk.Datarate);
        }
        break;
    case MODEM_LORA:
        {
                ...
                if( ( SX1272.Settings.FskPacketHandler.PreambleDetected == true ) && ( SX1272.Settings.FskPacketHandler.SyncWordDetected == false ) )
                {
...

                    {
                      int16_t Afc2CompVal =  ( int16_t )
                        ( ( ( uint16_t )SX1272Read( REG_AFCMSB ) << 8 ) |
                          ( ( uint16_t )SX1272Read( REG_AFCLSB ) << 0 ) );
                      SX1272.Settings.FskPacketHandler.AfcValue = ( int32_t )
